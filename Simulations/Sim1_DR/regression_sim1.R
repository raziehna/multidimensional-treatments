...
get_regression <- function(n, p, C, A, Y, dat, method, samp_size, A_sample, miss_Y){
 ...
 if (method == "GLM"){
  ...
  if (miss_Y){
    fmla_Y = as.formula(paste0(paste0("Y~-1+"), paste0("A", 1:p, collapse = "+"), "+", 
                               paste0(colnames(A2), collapse = "+"), "+ C1 + C2 + C3 + ",
  ...
  if (miss_Y){
    # E[Y | A, C]
    y_ac = A%*%beta_y[1:p] + A^2%*%beta_y[(p+1):(2*p)] + C[,1:3]%*%beta_y[(2*p+1):(2*p+3)] + A%*%beta_y[(2*p+4):length(beta_y)]*Csum
    
    # E[Y | Asamp, C]
    Yrep = Arep%*%beta_y[1:p] + Arep^2%*%beta_y[(p+1):(2*p)] + Crep[, 1:3]%*%beta_y[(2*p+1):(2*p+3)] + Arep%*%beta_y[(2*p+4):length(beta_y)]*rowSums(Crep)
