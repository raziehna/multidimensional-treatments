...
for (i in 1:iterations){
  ...
  Sigma = cov(dat)
  obj_A = get_fA_fAC(n, p, case, C, A, Y, dat, method_A, samp_size, miss_A) # bart vs dmvnorm
  fA_fAC = obj_A$fA_fAC
  A_sample = obj_A$A_sample

  # E[Y | A, C]
  obj_Y = get_regression(n, p, C, A, Y, dat, method_Y, samp_size, A_sample, miss_Y) # bart vs GLM
  y_ac = obj_Y$y_ac
  Yrep = obj_Y$Yrep
